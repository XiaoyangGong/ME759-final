#include "Stitcher_CUDA_opt.hpp"

using namespace std;
using namespace cv;
using namespace cv::cuda;
	// constructor
Stitcher_CUDA_opt :: Stitcher_CUDA_opt(){
	matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
	cv::cuda::printShortCudaDeviceInfo(cv::cuda::getDevice());
}
// inline
GpuMat Stitcher_CUDA_opt :: stitch(GpuMat& img1, GpuMat& img2){
    ...
    matcher->knnMatch(Mat(descriptors1), Mat(descriptors2), knn_matches, 2);
